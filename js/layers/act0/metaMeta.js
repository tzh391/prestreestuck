if (act == "0.2") addLayer("metaMeta", {
    ...
    prestigeButtonText() { 
        return "Ascend for <b>+" + formatWhole(tmp[this.layer].resetGain) + "</b> Metaness" +
         (Decimal.gte(tmp[this.layer].resetGain, 1000) ? "" : "<br/>Next at " + formatWhole(tmp[this.layer].nextAt) + " points")
    },
    getResetGain() {
    ...
    effect() {
        ...
        var tet = mtn.pow(mtt).add(1).log(10).mul(bns).mul(asf).mul(clf).pow(tmd.mul(ptb).div(nrf.div(player.metaMeta.sacrificeMulti)))
